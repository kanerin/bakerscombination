class UsersController < ApplicationController
    ...
    def show
        ...
        if params[:search_1] == nil
            @bakerscombinations= Bakerscombination.where(user_id: current_user.id).order(created_at: :desc).page(params[:page_1]).per(5)
        elsif params[:search_1] == ''
            @bakerscombinations= Bakerscombination.where(user_id: current_user.id).order(created_at: :desc).page(params[:page_1]).per(5)
        else
        #部分検索
            @bakerscombinations = Bakerscombination.where(user_id: current_user.id).where("name LIKE ? ",'%' + params[:search_1] + '%').order(created_at: :desc).page(params[:page_1]).per(5)
        end
